...
use crate::connection::{ConnectionAddr, ConnectionInfo, IntoConnectionInfo, PubSubSubscriptionInfo};
use crate::types::{ErrorKind, RedisError, RedisResult};
use crate::{cluster, cluster::TlsMode, ProtocolVersion, PushInfo};
...
pub struct ClusterParams {
    ...
    //pub(crate) pubsub_subscriptions: PubSubSubscriptionInfo,
}
...
impl ClusterClient {
    ...
    pub fn get_connection(&self, push_sender: Option<mpsc::UnboundedSender<PushInfo>>) -> RedisResult<cluster::ClusterConnection> {
        cluster::ClusterConnection::new(self.cluster_params.clone(), self.initial_nodes.clone(), push_sender)
    }
    ...
    /// An error is returned if there is a failure while creating connections or slots.
    #[cfg(feature = "cluster-async")]
    pub async fn get_async_connection(&self, push_sender: Option<mpsc::UnboundedSender<PushInfo>>) -> RedisResult<cluster_async::ClusterConnection> {
        cluster_async::ClusterConnection::new(&self.initial_nodes, self.cluster_params.clone(), push_sender)
            .await
    }

    #[doc(hidden)]
    pub fn get_generic_connection<C>(&self, push_sender: Option<mpsc::UnboundedSender<PushInfo>>) -> RedisResult<cluster::ClusterConnection<C>>
    where
    ...
    where
        C: crate::aio::ConnectionLike
            ...
            + 'static,
    {
        cluster_async::ClusterConnection::new(&self.initial_nodes, self.cluster_params.clone(), None)
            .await
    }
