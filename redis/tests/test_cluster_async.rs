    ...
    fn test_async_cluster_resp3_pubsub() {
        let client_subscriptions = PubSubSubscriptionInfo::from([
            (
                ...
                HashSet::from([PubSubChannelOrPattern::from(b"test_channel_?")]),
            ),
            (
                PubSubSubscriptionKind::Pattern,
                HashSet::from([
                    PubSubChannelOrPattern::from(b"test_*"),
                    PubSubChannelOrPattern::from(b"*"),
                ]),
            ),
            (
        ...
        block_on_all(async move {
            ...
            let mut subscribe_cnt = 0;
            let mut psubscribe_cnt = 0;
            let mut ssubscribe_cnt = 0;
            for _ in 0..(client_subscriptions[&PubSubSubscriptionKind::Exact].len()
                + client_subscriptions[&PubSubSubscriptionKind::Pattern].len()
                + client_subscriptions[&PubSubSubscriptionKind::Sharded].len())
            {
                let result = rx.try_recv();
                assert!(result.is_ok());
            ...
            assert!(subscribe_cnt == client_subscriptions[&PubSubSubscriptionKind::Exact].len());
            assert!(psubscribe_cnt == client_subscriptions[&PubSubSubscriptionKind::Pattern].len());
            assert!(ssubscribe_cnt == client_subscriptions[&PubSubSubscriptionKind::Sharded].len());
